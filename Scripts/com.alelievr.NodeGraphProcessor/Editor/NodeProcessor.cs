...
{
    public class NodeProcessor : BaseGraphProcessor
    {
        List< BaseNode >		processList;
        List< BuildNode >		startNodeList;
        ...
        public NodeProcessor(BaseGraph graph) : base(graph) {}

        public override void UpdateComputeOrder()
        {
            // Gather start nodes:
            startNodeList = graph.nodes.Where(n => n is BuildNode).Select(n => n as BuildNode).ToList();
        ...
        IEnumerable<BaseNode> GatherNonConditionalDependencies(BaseNode node)
        {
            ...
            while (dependencies.Count > 0)
            {
                ...
                foreach (var d in dependency.GetInputNodes().Where(n => !(n is Animator_INode)))
                    dependencies.Push(d);
	    ...
	    private IEnumerator<BaseNode> RunTheGraph(Stack<BaseNode> nodeToExecute)
		{
			...
			while(nodeToExecute.Count > 0)
			{
				...
				if(node is Animator_INode && !skipConditionalHandling.Contains(node))
				{

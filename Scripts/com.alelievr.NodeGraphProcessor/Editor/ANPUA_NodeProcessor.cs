...
{
    public class ANPUA_NodeProcessor : BaseGraphProcessor
    {
        List< BaseNode >		processList;
        List< ANPUA_StartNode >		startNodeList;
        ...
        public ANPUA_NodeProcessor(BaseGraph graph) : base(graph) {}

        public override void UpdateComputeOrder()
        {
            // Gather start nodes:
            startNodeList = graph.nodes.Where(n => n is ANPUA_StartNode).Select(n => n as ANPUA_StartNode).ToList();
        ...
        IEnumerable<BaseNode> GatherNonConditionalDependencies(BaseNode node)
        {
            ...
            while (dependencies.Count > 0)
            {
                ...
                foreach (var d in dependency.GetInputNodes().Where(n => !(n is ANPUA_INode)))
                    dependencies.Push(d);
	    ...
	    private IEnumerator<BaseNode> RunTheGraph(Stack<BaseNode> nodeToExecute)
		{
			...
			while(nodeToExecute.Count > 0)
			{
				...
				if(node is ANPUA_INode && !skipConditionalHandling.Contains(node))
				{
					...
					if(nodeDependenciesGathered.Contains(node))
					{
						...
						switch(node)
						{
							...
							case ANPUA_NodeForLoop forLoopNode:

...
{
	// Custom binding class to bind the field to the label
	public class FieldBinding : IBinding
	{
		private readonly BaseNode nodeTarget;
		private readonly FieldInfo fieldInfo;
		private readonly Label label;

		public FieldBinding(BaseNode nodeTarget, FieldInfo fieldInfo, Label label)
		{
		...
		{
			// No pre-update actions needed
		}

		public void Update()
		{
			// Update the label's text based on the field's value
			label.text = fieldInfo.GetValue(nodeTarget)?.ToString();
		}

		public void Release()
		{
			// No release actions needed
		}
	}
	...
	public class BaseNodeView : NodeView
	{
		...
		protected Label customNoteField;//Child of rightTitleContainer is a Label
		...
		void InitializeView()
		{
			...
			customNoteField = new Label { name = "custom-note" };
			customNoteField.text = nodeTarget.name;
			rightTitleContainer.Add(customNoteField);

			//Adjust Style of the customNoteField
			customNoteField.style.unityTextAlign = TextAnchor.MiddleRight;
			customNoteField.style.unityFontStyleAndWeight = FontStyle.Bold;
			customNoteField.style.color = new StyleColor(new Color(0.5f, 0.5f, 0.5f, 1f));
			//add padding
			customNoteField.style.paddingRight = 2.5f;
			customNoteField.style.paddingLeft = 2.5f;
		...
		{
			var titleLabel = this.Q("custom-note") as Label;//"title-label" "custom-note" to implement the Editing to the customNoteField for Now
		...
		void UpdateTitle()
		{
			...
			title = nodeTarget.GetName();
			customNoteField.text= (nodeTarget.GetCustomOrEmpty() == null) ? "   " : nodeTarget.GetCustomOrEmpty();
		}
